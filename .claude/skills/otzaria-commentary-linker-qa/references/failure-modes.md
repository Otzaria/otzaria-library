...
| **F18** | **`path_2` resolves to no book** | The target title in `path_2` isn't `book.title` in `seforim.db` character for character, so the generator drops every entry with that `path_2` — **silently: no error, no warning, no log line, the links simply never exist in the DB**. Structural QA, heRef QA and semantic QA can all be green while 100% of the file is dead. Almost always gershayim, and it cuts both ways: a Sefaria target written the on-disk way (`רשי על שבת.txt` where the book is `רש"י על שבת`), **or** an Otzaria-native target written with `"`/`''` where the importer's `normalizeBookTitle` folded the title to `״` (`הערות על וזה לשונו - שובבי''ם.txt` where the book is `הערות על וזה לשונו - שובבי״ם`). Neither the filename nor the book's source is the authority: Otzaria-native titles always carry `״`, Sefaria titles keep Sefaria's own spelling (usually ASCII `"`, sometimes `״`) — only `book.title` decides. Confirm with `SELECT title FROM book WHERE title = ?` (exact, **not** `LIKE`) for **every distinct** `path_2`; zero rows = `blocker`. Real cases: 739 entries across 33 files (Sefaria target), 116 entries in `MoreBooks` (Otzaria target) |
...
8. **F18:** collect the set of distinct `path_2` values first (a one-liner over the JSON), then
   exact-match each against `book.title`. Check both directions: stripped gershayim where the DB
   has `"` (Sefaria target), and `"`/`''` where the DB has `״` (Otzaria-native target). If a miss
   is a gershayim variant, the fix is the DB spelling — never rename the target or drop to
   `LIKE` to make it "match".
