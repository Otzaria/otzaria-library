...
find_seforim_child() {
  ...
  if ! rows=$(TITLE="$title" gh api --paginate -X GET \
      ...
      --jq '.workflow_runs[] | select(.display_title==env.TITLE) | [(.id|tostring),.status,(.conclusion//"__ACTIVE__"),.head_sha] | @tsv'); then
  ...
  while IFS=$'\t' read -r id status conclusion head; do
    [ -n "$id" ] || continue
    [[ "$id" =~ ^[1-9][0-9]*$ && "$head" =~ ^[0-9a-f]{40}$ ]] || {
      echo "::error::invalid Seforim child identity in exact-title results" >&2
      return 2
    }
...
ensure_continuation() {
  ...
  if ! rows=$(TITLE="$title" HEAD_SHA="$CONTROL_HEAD" gh api --paginate -X GET \
      ...
      --jq '.workflow_runs[] | select(.display_title==env.TITLE and .head_sha==env.HEAD_SHA) | [(.id|tostring),.status,(.conclusion//"__ACTIVE__"),(.run_attempt|tostring)] | @tsv'); then
