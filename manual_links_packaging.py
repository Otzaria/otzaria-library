...
def target_title_or_none(value: object) -> str | None:
    """Mirrors ``targetTitleOrNull``: only a ``.txt`` component names a book."""
    if not isinstance(value, str):
        return None
    component = value.replace("\\", "/").rsplit("/", 1)[-1]
    if not component.endswith(BOOK_SUFFIX) or len(component) <= len(BOOK_SUFFIX):
        return None
    return component[: -len(BOOK_SUFFIX)]
